...
class FileNotFoundDialog():
  ...
  def __init__(self, file, parent):
    ...
    self.parent = parent
    self.builder = Gtk.Builder.new_from_resource("/de/geigi/cozy/file_not_found.ui")
    self.dialog = self.builder.get_object("dialog")
    self.builder.get_object("file_label").set_markup("<tt>" + file + "</tt>")
  ...
  def locate(self, button):
    ...
    if response == Gtk.ResponseType.OK:
      ...
      Track.update(file = new_location).where(Track.file == self.missing_file).execute()
      self.dialog.destroy()
